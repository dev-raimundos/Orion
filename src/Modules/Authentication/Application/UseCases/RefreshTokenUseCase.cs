    ...
    ITokenGenerator tokenGenerator)
{
    ...
    public async Task<RefreshTokenResult> ExecuteAsync(RefreshTokenRequest request, CancellationToken ct)
    {
        ...
        var newRefreshToken = RefreshToken.Create(existing.UserId, existing.Email, newTokenHash, RefreshTokenPolicy.Lifetime);
        await _refreshTokens.AddAsync(newRefreshToken, ct);

        var (accessToken, expiresAt) = _tokenGenerator.Generate(existing.UserId, existing.Email);

        return new RefreshTokenResult(accessToken, expiresAt, newRawToken);
    }
}
