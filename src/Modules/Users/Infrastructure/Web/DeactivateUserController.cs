...
public class DeactivateUserController(DeactivateUserUseCase deactivateUser) : ControllerBase
{
    ...
    public async Task<ActionResult<DeactivateUserOutput>> Deactivate(Guid id, CancellationToken ct)
    {
        this.EnsureIsCurrentUser(id);

        var result = await _deactivateUser.ExecuteAsync(new DeactivateUserInput(id), ct);
        return Ok(result);
    }
}
